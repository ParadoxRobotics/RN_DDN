...
class ResBlock(nn.Module):
    def __init__(self, inchannel, outchannel, stride=1, shortcut=None):
        ...
        self.block = nn.Sequential(
            ...
            nn.Conv2d(inchannel, outchannel, 3, stride, 1, groups=inchannel, bias=False),
            nn.BatchNorm2d(outchannel),
            nn.ReLU(inplace=True),
            #nn.Conv2d(outchannel, outchannel, 3, 1, 1, bias=False),
            nn.Conv2d(outchannel, outchannel, 3, stride, 1, groups=outchannel, bias=False),
            nn.BatchNorm2d(outchannel)
        )
...
class RN_DDN(nn.Module):
    def __init__(self):
        super(RN_DDN, self).__init__()
        # input layer
    ...
    def forward(self, X_A, X_B):
        out_A = self.forward_inference(X_A)
        out_B = self.forward_inference(X_B)
        return out_A, out_B

# Network instantiation and test
RN_DDN = RN_DDN()
print("RN_DDN STRUCTURE : \n", RN_DDN)
total_params = sum(p.numel() for p in RN_DDN.parameters())
trainable_params = sum(p.numel() for p in RN_DDN.parameters() if p.requires_grad)
...
optimizer = optim.SGD(RN_DDN.parameters(), lr = 0.01,
                                            momentum = 0.90,
