...
class VisualDescriptorNet(torch.nn.Module):
    def __init__(self, descriptorDim, trainingMode):
        ...
        self.fullResNet = models.resnet50(pretrained=trainingMode)
        ...
        self.upConv4 = nn.Conv2d(256, self.descriptorDim, kernel_size=1)
        self.upConv8 = nn.Conv2d(512, self.descriptorDim, kernel_size=1)
        self.upConv16 = nn.Conv2d(1024, self.descriptorDim, kernel_size=1)
        self.upConv32 = nn.Conv2d(2048, self.descriptorDim, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = self.ResNet[4](x) # layer1 size=(N, 256, x.H/4, x.W/4)
        up1 = self.upConv4(x)
        x = self.ResNet[5](x) # layer2 size=(N, 512, x.H/8, x.W/8)
        up2 = self.upConv8(x)
        x = self.ResNet[6](x) # layer3 size=(N, 1024, x.H/16, x.W/16)
        up3 = self.upConv16(x)
        x = self.ResNet[7](x) # layer4 size=(N, 2048, x.H/32, x.W/32)
        up4 = self.upConv32(x)
